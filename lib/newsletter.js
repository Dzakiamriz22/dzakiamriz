...
function buildNotificationEmail({ title, excerpt, url }) {
  const safeTitle = title || 'Update';
  const safeExcerpt = excerpt || 'There is an update.';

  return {
    subject: `Update: ${safeTitle}`,
    html: `
      <div style="font-family:Arial,sans-serif;line-height:1.6;color:#0f172a;max-width:620px;margin:0 auto;padding:24px;">
        <p style="font-size:12px;letter-spacing:0.16em;text-transform:uppercase;color:#475569;margin-bottom:8px;">Update</p>
        <h1 style="font-size:24px;line-height:1.2;margin:0 0 12px;">${safeTitle}</h1>
        <p style="margin:0 0 16px;color:#334155;">${safeExcerpt}</p>
        ${url ? `<a href="${url}" style="display:inline-block;padding:10px 16px;background:#1d4ed8;color:#ffffff;text-decoration:none;font-weight:700;">Open</a>` : ''}
      </div>
    `,
  };
}
...
function buildSubscriptionSuccessEmail() {
  return {
    subject: 'Subscription confirmed: you will receive updates',
    html: `
      <div style="font-family:Arial,sans-serif;line-height:1.6;color:#0f172a;max-width:620px;margin:0 auto;padding:24px;">
        ...
        <p style="margin:0 0 16px;color:#334155;">Thanks for subscribing. You will receive email updates when there is new content or announcements.</p>
      </div>
    `,
  };
}
...
export async function notifySubscribers({ title, excerpt, url }) {
  ...
  const emailPayload = buildNotificationEmail({ title, excerpt, url });
  let sent = 0;
  const failed = [];
